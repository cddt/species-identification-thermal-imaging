...
def define_model_convLSTM():

    convLSTM = Sequential()
    convLSTM.add(ConvLSTM2D(32, (3,3), data_format = "channels_last", input_shape = (45,24,24,3), return_sequences = True))
    convLSTM.add(Dropout(0.3))
    #convLSTM.add(MaxPooling3D(pool_size=(2,2,2)))
    convLSTM.add(ConvLSTM2D(64, (3,3), data_format = "channels_last", input_shape = (45,24,24,3), return_sequences = True))
    convLSTM.add(Dropout(0.3))
    convLSTM.add(MaxPooling3D(pool_size=(1,5,5)))
    convLSTM.add(Reshape((45, 1024)))
    #convLSTM.add(Flatten())
    #convLSTM.add(Dense(256, activation="relu"))
    #convLSTM.add(Dropout(0.3))
    ...
    vid_features = convLSTM(vid_inputs)
...
# create log dir
if not os.path.exists("./logs/ConvLSTM"):
    os.makedirs("./logs/ConvLSTM")

current_time = str(datetime.datetime.now())

# csv logs based on the time
csv_logger = CSVLogger('./logs/ConvLSTM/log_' + current_time + '.csv', append=True, separator=';')
...
checkpointer = ModelCheckpoint(filepath='./logs/ConvLSTM/best_model_' + current_time + '.hdf5', verbose=1, save_best_only = True, monitor = 'val_accuracy', mode = 'max')
...
fig.savefig('./logs/ConvLSTM/plot' + current_time + '.svg', format = 'svg')

model.load_weights('./logs/ConvLSTM/best_model_' + current_time + '.hdf5')
