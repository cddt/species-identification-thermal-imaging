...
# create log dir
if not os.path.exists("./logs/LRCN"):
    os.makedirs("./logs/LRCN")

current_time = str(datetime.datetime.now())

# csv logs based on the time
csv_logger = CSVLogger('./logs/LRCN/log_' + current_time + '.csv', append=True, separator=';')
...
checkpointer = ModelCheckpoint(filepath='./logs/LRCN/best_model_' + current_time + '.hdf5', verbose=1, save_best_only = True, monitor = 'val_accuracy', mode = 'max')
...
train_data = DataGenerator(X_train, None, y_train, batch_size, True, 10, 0, 0)
...
fig.savefig('./logs/LRCN/plot' + current_time + '.svg', format = 'svg')

model.load_weights('./logs/LRCN/best_model_' + current_time + '.hdf5')
