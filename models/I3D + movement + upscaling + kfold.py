...
if not os.path.exists("./logs/kfold_plotdata" + current_time):
    os.makedirs("./logs/kfold_plotdata" + current_time)
...
def get_model(key):
    ...
    models['i3d-9'] = define_joint_model(upscaling, d_1 = 0.5, d_2 = 0.2, d_3 = 0.2, f_1 = 256, r_1 = 512, r_2 = 512, pooling = 'avg') #oops forgot to set f_1 to 512
    ...
    models['i3d-20'] = define_joint_model(upscaling, d_1 = 0.5, d_2 = 0.2, d_3 = 0.2, f_1 = 512, r_1 = 512, r_2 = 512, pooling = 'avg') #instead of i3d-9
...
def evaluate_model(model, name):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X = np.zeros(X_train2.shape[0]), y = y_train2.argmax(1))): # because split returns incidies we only need one
        ...
        checkpointer = ModelCheckpoint(filepath='./logs/kfold' + current_time + '/best_weights_fold' + str(fold + 1) +'_'+name + '.hdf5', verbose=1, save_best_only=True, monitor='val_accuracy', mode = 'max')
...
#    models.pop(name, None)
    ...
    names.append(name)
    with open('./logs/kfold_plotdata' + current_time + '/plotdata_box_acc' + current_time + '_' + name + '.csv', "wb") as fp:
        pickle.dump(box_acc, fp)
    with open('./logs/kfold_plotdata' + current_time + '/plotdata_box_loss' + current_time + '_' + name + '.csv', "wb") as fp:
        pickle.dump(box_loss, fp)
    with open('./logs/kfold_plotdata' + current_time + '/plotdata_names' + current_time + '_' + name + '.csv', "wb") as fp:
