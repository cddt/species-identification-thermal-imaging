...
"""
Classifies using solely the movement information.
"""

def load(name):
    # Loads the videos and converts the labels into one-hot encoding for Keras
    X = np.load("./cacophony-preprocessed-movement/" + name + ".npy")
    y = np.load("./cacophony-preprocessed-movement/" + name + "-labels.npy")
...
model = Sequential()
model.add(LSTM(256, return_sequences = False, input_shape = (45, 9)))
