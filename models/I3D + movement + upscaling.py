...
def define_joint_model(upscaling):
    ...
    mvm_features = LSTM(512, return_sequences=True, dropout = 0.2, recurrent_dropout = 0.0)(mvm_inputs)
    ...
    x = LSTM(512, return_sequences = True, dropout = 0.2, recurrent_dropout = 0.0)(x)
...
model = define_joint_model(upscaling)
...
checkpointer = ModelCheckpoint(filepath='./logs/I3D/best_model_acc_' + current_time + '.hdf5', verbose=1, save_best_only = True, monitor = 'val_accuracy', mode = 'max')

checkpointer2 = ModelCheckpoint(filepath='./logs/I3D/best_model_loss_' + current_time + '.hdf5', verbose=1, save_best_only = True, monitor = 'val_loss', mode = 'min')

# Training the model on the training set, with early stopping using the validation set
callbacks = [EarlyStopping(patience = 10), reduce_lr, csv_logger, checkpointer, checkpointer2]
...
model.load_weights('./logs/I3D/best_model_acc_' + current_time + '.hdf5')
...
model.load_weights('./logs/I3D/best_model_val_' + current_time + '.hdf5')
