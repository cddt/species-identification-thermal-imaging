...
class DataGenerator(Sequence):
    def __init__(self, vids, labels, batch_size, flip = False, angle = 0, crop = 0, shift = 0, shuffle = True):
        self.vids = vids
        self.labels = labels
    ...
    def __getitem__(self, index):
        ...
        return vids, self.labels[indices]
    
    def on_epoch_end(self):
        if self.shuffle:
            np.random.shuffle(self.indices)
            
train_data = DataGenerator(X_train, y_train, batch_size, True, 10, 0, 0)
val_data = DataGenerator(X_val, y_val, batch_size)
